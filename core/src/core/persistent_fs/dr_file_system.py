...
class DRFileSystem(DataRobotFileSystem):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Paths known to have no copy in the legacy filesystem. Used to skip the
        # KeyValue-backed legacy existence probes on the write path. Only absences
        # are cached: the new filesystem is the sole write target, so a path never
        # gains a legacy copy, and a path only ever migrates from legacy to new
        # (see `_open`), so a cached absence stays valid for this instance.
        self._paths_absent_from_legacy: set[str] = set()
    ...
    def _open(  # type: ignore[override]
        ...
        if mode == "rb":
            ...
            legacy_key = self._strip_protocol(path)
            if legacy_key in self._paths_absent_from_legacy:
                # Cached absence: skip the (KeyValue-backed) legacy existence
                # probes entirely. Repeated writes to the same path (e.g. a
                # persisted SQLite DB) hit this after the first upload.
                has_legacy_file = False
            else:
                has_legacy_file = self._legacy_fs.exists(
                    path
                ) and self._legacy_fs.isfile(path)
                if not has_legacy_file:
                    self._paths_absent_from_legacy.add(legacy_key)
            ...
            def upload_and_cleanup(final: bool = False) -> bool:
                ...
                # The legacy copy is gone; cache the absence so subsequent writes
                # to this path skip the probe.
                self._paths_absent_from_legacy.add(legacy_key)
                return file_was_uploaded
